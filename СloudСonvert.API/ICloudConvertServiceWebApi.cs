...
namespace СloudСonvert.API
{
  public interface ICloudConvertServiceWebApi
  {
    ...
    /// </returns>
    [Post("/{operation}")]
    Task<TaskResponse> CreateTask<T>([Header("Authorization")] string api_key, string operation, [Body(BodySerializationMethod.Serialized)] T request);
    ...
    [Multipart]
    [Post("/import/upload")]
    Task<TaskResponse> Upload([Header("Authorization")] string api_key, StreamPart file, TaskResponse task);
